...
```{r, echo=FALSE, fig.cap="**Left:** Ovarian Cancer Cell Lines Mean Sensitivity towards Drugs  **Right:** Mean Drug Efficiency regarding Proliferation in Ovarian Cell Lines", fig.height=4, fig.width=7}
...
```{r, echo=FALSE, fig.cap="**Left:** Most Effective Drug-Proliferation Values in Ovarian Cell Lines  **Right:** Most frequent moas amongst the most effective drugs", fig.height=3, fig.width=8}
...
In order to be able to work with a manageable amount of drugs, we decided to set a threshold at the 10 % quantile (see Fig. 2) By doing so, we could focus on the 140 most effective drugs which caused the most negative proliferation values in the ovarian cancer cell lines.
...
Our results are shown in Fig. 3. These are the 45 most common moas regarding the drugs that had the most promising effects on the ovarian cancer cell lines.
...
```{r, echo=FALSE, fig.cap=" **Left:** Variance Explained by Mechanism of Action PCs  **Right:** Elbow Method for Finding Optimal Number of Clusters", fig.height=4, fig.width=7.5}
...
Fig. 4 shows that the majority of the variance is explained by using the first two principal components.
In order to find the most optimal number of clusters for our PCA of PC1 and PC2, we used the elbow method. As a result, the optimal number of clusters seems to be 4 (see Fig. 4).


```{r, echo=FALSE, fig.cap = "**Left:** PC1 vs. PC2 for Ovary Cell Line Clusters  **Right:** PC1 vs. PC2 for CN Values in Ovary Cell Lines", fig.height=4.5, fig.width=8}
...
Fig. 5 shows the clustering for the ovarian cell lines according to their reactions towards the 45 selected drugs (see Fig. 3). They did not cluster in completely distinct clusters but one cell line (ACH-000524) forms its own cluster which is why we looked further into that one. 
According to the data set prism.cl, the outlier cell line belongs to the most prominent lineage subtype ovary adenocarcinoma. This subtype makes up 28 of 30 lineage subtypes in our data with the other two being brenner tumor and ovary carcinoma. This is why we figured that the lineage subtype could not have been the reason for the cell line forming its own cluster.
...
However, the cell line showed a different behavior and clustered separately from the majority according to its gene copy number (CN) values (see Fig. 6). This led us to the conclusion that either, the proliferation values of that cell line originated from measuring errors or that its differing behavior (see Fig. 5) is a result of distinct CN values. 
...
In Fig. 7, the contribution to PC1 for ovary cell line clusters (see Fig. 5) is shown. Drugs with values close to zero show a similar effect on the proliferation amongst all cell lines - thus, these are generally effective when administered to ovary cancer cell lines. This is the case for the following drugs (see highlighted in Fig. 7). 

The ATPase inhibitor oubain is normally used to treat hypertension and cardiac arrhythmia, however, a study found that it can have promising effects as a cancer treatment for ER+ breast cancer [@Chen2006]. In another study, researchers found that one in three ovarian cancers are ER+ [@Harding1990], which leads to the conclusion that this is why we found that ouabain generally showed positive effects amongst the ovarian cancer cell lines. 

The drugs JNJ-26481585 [@Arts6841] and romidepsin [@doi:10.1517/13543784.2011.594437] are HDAC inhibitors that, as mentioned before, in general show promising effects on ovarian cancer cell lines. Therefore, it makes sense that we found them as one of the five most effective drugs regarding general effectiveness on all cell lines. 
...
Amsacrine, a topoisomerase inhibitor, is a chemotherapy drug that is commonly used for acute leukemia as well as Hodgkin’s and non-Hodgkin’s lymphomas [@Ketron2012]. Hence, a general effect on the proliferation of ovarian cancer cell lines seems to be logical. 
