...
load("/Users/laura/OneDrive/Dokumente/Uni/Bioinfo/Data Project/Original_Environment/prism_datasets.rda")
load("/Users/laura/OneDrive/Dokumente/Uni/Bioinfo/Data Project/Original_Environment/cellline_datasets.rda")
```
...
```{r, echo=FALSE, fig.cap = "Most frequent MOA's amongst the most effective drugs", fig.height=3, fig.width=4}
...
After finding the drugs with the most effective response in regards to the ovary cancer cell lines, we now continued our analysis by testing if the efficiency of these drugs is dependent on the cancer cell line type or not. In order to test this we repeated many of the same steps we completed for the ovary cell lines but on the non-ovary cell lines. We extracted the most negative proliferation value for each drug in each non-ovary cell line, set the threshold to the 10% quantile, and then extracted the most common MOAs.
...
```{r, echo = FALSE, fig.cap = "PC1 vs. PC2 for All Cell Line Clusters", fig.height=4.5, fig.width=5}
...
```
Additionally to the PCA clustering in Fig. 4.a. and the PCA clustering above, we also conducted a PCA using only non-ovary cell lines (not shown). As a final step to find out if the drug response is related to the cell line type, we decided to perform a Welch's unpaired two sided t-test, which was subjected a Bonferroni correction. In order to do this we reduced the prism.treat data frame further. It now included only those cell lines located in clusters two and four from the PCA clusterings for non ovary and ovary cell lines, respectively. With these two data frames we performed a Welch's unpaired two sided t-test, which was subjected a Bonferroni correction. The resulting p-value was higher than the confidence level of 0.05 which means that the null hypothesis - that there is no association between the mean value of the drug response in ovary and non ovary cell lines - could not be rejected. This shows once again that the drug response seems to be independent of the cell line type in regards to the ovary or non-ovary cell lines.

# DRUG DOSAGE DEPENDENCY

## BOXPLOT OF PROLIFERATION VALUES 
```{r out.width = "70%", echo = FALSE, fig.cap="Boxplot of the distribution of the proliferation rates of ovary cell lines according to the drug dosage", fig.height = 3, fig.width = 8, fig.align = 'center', cex.axis = 0.75}
...
```
We implied a boxplot of the distribution of the ovary cell proliferation rates, regarding the drug dose. We could not divide the boxplot by the 2.5 micromolar dosage (Corsello et al. 2020), but rather in 3 stages: low, median and high dose, corresponding with a highest median, median and lowest proliferation rate, respectively. Consequently, an increasing of drug dose leads to reduction of proliferation rates.

## PPEARSON CORRELATION
Almost all Pearson correlation coefficients are negative, except for one between 'ACH-000713' and 'exatecan-mesylate', which is positive and thus is an outlier. Consequently,some drugs have a quite strong negative correlation with the proliferation, while others do not affect it.

## VISUALIZATION - GGPLOT
Furthermore, we created correlation matrix from the dosages of the different drugs per cell line and displayed the Pearson correlation. The blue and the purple squares indicate higher correlation, meaning that the majority of the drugs reduce the cell proliferation.

```{r out.width = "70%",echo=FALSE, error=FALSE, fig.cap="ggplot of the correlation between the proliferation rates of ovary cell lines and the dosage of the applied medical treatment", fig.height=5,fig.weight=9, message=FALSE, warning=FALSE, results='hide', fig.align='center', cex.axis = 0.75}
...
## DRUG RESPONSE DEPENDENCY ON GENE EXPRESSION PATTERNS
## PROCESS AND RESULTS
...
There are significant differences in the drug response for the drugs - "torin-1","AVN-944" for the clusters 2-3 and "trichostatin-a","semaxanib","alvocidib" for the clusters 1-3.
...
knitr::kable(pvalues_t_test_treat3vs1_reduced,
           caption = "Table of the 3 drugs with a significant different response for clusters 1 and 3")
t1 <-kable(pvalues_t_test_treat3vs1_reduced, format = "latex", booktabs = TRUE)
knitr::kable(pvalues_t_test_treat3vs2_reduced,
             caption = "Table of the 2 drugs with a significant different response for clusters 2 and 3")
t2 <-kable(pvalues_t_test_treat3vs2_reduced, format = "latex", booktabs = TRUE)
#knitr::kable(pvalues_t_test_treat3vs2_reduced,
          #  caption = "Table of the drugs with a significant different response for clusters 1,2 and 3")
