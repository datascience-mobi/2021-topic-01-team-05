...
sum(which(unlist(hist_cell_growth_drugs) < 0)) # number of values below zero in hist_cell_growth_drugs
...
library(dplyr)
reduced_prism_treat_ovary_df_2 %>% dplyr::count(moa)
...
par(mar=c(4,20,4,4))
barplot(most_common_moas_ovary$n, names.arg = most_common_moas_ovary$moa, las = 1, xlab = "Frequency", horiz=TRUE)
...
barplot(sort(pca2_moa_t$rotation[,1]), las=3, cex.names = 0.4)
...
as.list(sort(pca2_moa$rotation[,2]) < 0)
...
## Creating a copy of prism_reduced_all to keep a version that includes the NAs
...
## Checking if the ovary cell lines also have NFkB inhibitor moa

as.vector(na.omit((reduced_prism_treat_ovary_df_2[reduced_prism_treat_ovary_df_2$moa == "NFkB pathway inhibitor",])$name)) ## has two
...
## creating a bar plot showing the p and odds ratio values
...
```{r S17 PCA on prism.cnv}

A_cnv = as.matrix(prism.cnv)

pca_cnv = prcomp(A_cnv, scale = TRUE)
