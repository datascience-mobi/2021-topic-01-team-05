...
```{r S4 Schwellenwert festlegen um unsignifikante Medikamente aus prism_reduced zu entfernen}
...
hist(min_values_treat_prism_reduced_vector, main = "Histogramm of the lowest proliferation values in prism_reduced")
...
par(mar=c(4, 20, 4, 4))
barplot(moa_frequencies$n, names.arg = moa_frequencies$moa, las = 1, xlab = "Frequency", horiz = TRUE, xlim = c(0,10), main = "Most frequent moa's amongst the medications under the threshhold") ## The 3 NAs still have to be removed here, but I don't know how
## This still needs to be organized by frequency but the values and names are not connected


```

```{r S9 Finding out in which celllines the HDAC inhibitor shows up}
...
```{r S 10 Chunk 12 for topoisomerase inhibitor}
...
```{r S11 Finding the names of the medications from the barplot in chunk r S11 Analysing our data}
...
```{r S13 Clustering moa_drugs by cell lines}
...
```{r S14 Clustering moa_drugs by medications}
