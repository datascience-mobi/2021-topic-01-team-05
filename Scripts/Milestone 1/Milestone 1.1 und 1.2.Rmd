...
```{r c1 - Data Exploration and Organization - Reducing the prism.cl data set to obtain ovary cancer values}
...
## Subsetting df_cl in order to extract only ovary cell lines with their DepMap_IDs.
...
## Creating a subset of prism containing only the ovary DepMap_IDs and their proliferation values. 

df_ovary_DepMI_prism  <- prism[rownames(prism) %in% vector_ovary_DepMI,]
```


```{r c3 - Data Exploration and Organization - Histograms of prism focusing only on ovary cell lines}

## Computing histograms in order to visualize the distribution of the mean sensitivity across all ovary cell lines and all drugs.

hist_cell_growth_cl  <- hist(apply(df_ovary_DepMI_prism, 1,function(x){mean(x,na.rm=TRUE)}), breaks = 50, xlim = c(-0.9, 0),  main = "Sensitivity in Ovary Cell Lines", xlab = "Mean log fold-change in Cell Growth")

hist_cell_growth_drugs <- hist(apply(df_ovary_DepMI_prism, 2,function(x){mean(x,na.rm=TRUE)}), breaks = 50, main = "Drug Effects on Proliferation in Ovary Cell Lines", xlab = "Mean log fold-change in Cell Growth")
```

```{r c4 - Data Exploration and Organization - Adapting the prism data set in order to find the most effective drugs regarding ovary cancer cell line proliferation}

## Creating a data frame that contains the most effective treatment values for each drug and dosage (min values) for every ovary cell line 
## This requires a code that prints the min values for every drug and the corresponding dosages since the min values correspond to the strongest negative effects on proliferation 
...
## prism_reduced_ovary contains every drug and its strongest effects (regardless of the specific dosage that was used) on each ovary cancer cell line
```


```{r c4.1 - Data Exploration and Organization - unlist prism_reduced_ovary in order to compute a histogram}
...
```{r c4.15. - visualizing the clusters; clustering by medications}
...
#legend("right", pca2_moa_t$x[,1:2], legend = km2_moa_t$cluster, col = col.km2_moa_t, pch=20, fill = col)
...
library(ggplot2) ## didnt end up using this here

#Try 2
hist_prism_reduced <- hist(unlist(prism_reduced), xlab = "Proliferation values", main = "Proliferation values of prism_reduced", breaks = 100) 
...
## For loop that replaces the NAs with the column means
...
## Plotting the proliferation values from prism_reduced_no_NAs
hist(unlist(prism_reduced_no_NAs), breaks = 100, xlab = "Proliferation values", main = "Proliferation values of prism_reduced_no_NAs")
```

```{r S4 Setting a threshhold in order to remove the unimportant medications from prism_reduced}
...
## Creating a histogramm displaying the proliferation values under the treshhold
hist(min_values_treat_prism_reduced_vector)
...
```{r S5 Creating a vector containing the min proliferation values of prism_reduced under the treshhold}
...
```{r S6 Finding the cell lines corresponding to the lowest proliferation value of each drug with prism.treat}
...
```{r Finding the names of the medications from the barplot in chunk r S11 Analysing our data}
...
```{r S15 k means clustering prism_reduced_no_NAs by cell lines}
...
```{r k means on prism_reduced_no_NAs by medications}
...
## Silhouette plot 
D1_all = dist(x=t(prism_reduced_no_NAs))

library(cluster)
km1_all = kmeans(x=t(prism_reduced_no_NAs), centers = 4, nstart = 10)
...
par(mfrow = c(2,3))
for (i in 3:14){
  km1_all = kmeans(x=t(prism_reduced_no_NAs), centers = i, nstart = 20)
  s1_all = silhouette(km1_all$cluster, D1_all, col="red")
  plot(s1_all, border = NA)
}
...
```{r - r1 performing PCA of the datasets prism_reduced_no_NAs  }
...
plot(pca1$x[,1], pca1$x[,2], xlab="PC1", ylab = "PC2", main = "PC1 / PC2 - plot", pch=20)
...
# Plot of PC1 against PC2
plot(pca2$x[,1], pca2$x[,2], xlab="PC1", ylab = "PC2", main = "PC1 / PC2 - plot", pch=20)
...
km_all = kmeans(pca1$x[,1:2],centers=5,nstart = 100)
...
km1_all = kmeans(pca1$x[,1:2],centers=5,nstart = 100)
col = c('green','blue','red', "orange", "brown")
...
```{r Clustering moa_drugs by cell lines}
...
```{r Clustering moa_drugs by medications}
