...
#names(which(table(reduced_prism_treat_ovary_df_2$moa) > 1))


#barplot(which(table(reduced_prism_treat_ovary_df_2$moa) > 1), las=3)
##
...
testdf <- reduced_prism_treat_ovary_df_2 %>% count(moa) %>% filter(n >= 2)

par(mar=c(4,20,4,4))
barplot(testdf$n, names.arg = testdf$moa, las = 1, xlab = "Frequency", horiz=TRUE)
```

```{r - c4.8 - analyzing HDAC inhibitors}
...
as.vector(na.omit((reduced_prism_treat_ovary_df_2[reduced_prism_treat_ovary_df_2$moa == "HDAC inhibitor",])$broad_id)) 

## Extracting the cell lines from prism using the above extracted broad IDs
prism[prism$colnames == as.vector(na.omit((reduced_prism_treat_ovary_df_2[reduced_prism_treat_ovary_df_2$moa == "HDAC inhibitor",])$broad_id))]
