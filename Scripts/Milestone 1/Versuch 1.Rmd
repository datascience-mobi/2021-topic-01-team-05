...
## k means clustering by medications
...
D = dist(prism_reduced_no_NAs)

library(cluster)
km = kmeans(prism_reduced_no_NAs, centers = 4, nstart = 10)
s = silhouette(km$cluster, D, col="red")
plot(s, border = NA)
...
par(mfrow = c(2,3))
for (i in 3:14){
  km = kmeans(prism_reduced_no_NAs, centers = i, nstart = 20)
  s = silhouette(km$cluster, D, col="red")
  plot(s, border = NA)
}
...
# Error is shown
# Remove NAs from prism_reduced_ovary_NAs
...
# Count NAs in prism_reduced_ovary_no_NAs
print(paste(c("Percentage of NAs:", round(sum(apply(prism_reduced_ovary_no_NAs, 2, function(x){sum(is.na(x))})) / (nrow(prism_reduced_ovary_no_NAs)*ncol(prism_reduced_ovary_no_NAs)),2)), collapse = " "))
