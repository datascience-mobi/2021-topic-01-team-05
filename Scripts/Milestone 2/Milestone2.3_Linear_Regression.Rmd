...
regression_data <- prism_reduced_ovary_no_NAs["zardaverine"]
...
```{r}
regression_model <- lm(formula = zardaverine ~  .,data=learning_regression)
...
cor(learning_regression$zardaverine, regression_model$residuals)





plot(learning_regression$zardaverine, regression_model$fitted.values,pch=20, col="light blue", xlab = "Control", ylab = "Residuals");abline(0,1,col="red")
```
...
plot(check_regression$zardaverine, proliferation_pred,pch=20, col="lightblue3", xlab = "Real values", ylab = "Predicted values");abline(0,1,col="red" )
...
residuals = check_regression$zardaverine - proliferation_pred
