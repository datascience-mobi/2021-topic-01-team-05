...
##for ovary
df_ovary_prism_treat_effective_longc2c4 <- df_ovary_prism_treat_effective_longc2c4 [df_ovary_prism_treat_effective_longc2c4$name%in%c("exatecan-mesylate","romidepsin","carfilzomib"),]
df_ovary_prism_treat_effective_longc2c4

##using pivot wider
df_ovary_prism_treat_effective_c2c4<-pivot_wider(df_ovary_prism_treat_effective_longc2c4, names_from = DepMap_ID,values_from= proliferationvalues)
df_ovary_prism_treat_effective_c2c4
...
##deleting every drug except for exatecan-mesylate, romidepsin and carfilzomib
##for non ovary
df_noovary_prism_treat_effective_longc2c4 <- df_noovary_prism_treat_effective_longc2c4 [df_noovary_prism_treat_effective_longc2c4$name%in%c("exatecan-mesylate","romidepsin","carfilzomib"),]
df_noovary_prism_treat_effective_longc2c4

##using pivot wider
df_noovary_prism_treat_effective_c2c4<-pivot_wider(df_noovary_prism_treat_effective_longc2c4, names_from = DepMap_ID,values_from= proliferationvalues)
df_noovary_prism_treat_effective_c2c4
...
##ovary
ovary_achilles_cluster24mean<-data.frame(apply(ovary_achilles_cluster24,2,mean, na.rm=TRUE))
ovary_achilles_cluster24mean


which(ovary_achilles_cluster24mean==min(ovary_achilles_cluster24mean), arr.ind =TRUE)
select(ovary_achilles_cluster24, RAN)

##non ovary
noovary_achilles_cluster24mean<-data.frame(apply(noovary_achilles_cluster24,2,mean, na.rm=TRUE))
noovary_achilles_cluster24mean

which(noovary_achilles_cluster24mean==min(noovary_achilles_cluster24mean), arr.ind =TRUE)
select(noovary_achilles_cluster24, RAN)

##creating a dataframe with the delta between the ovary and non ovary proliferationvalues
delta_ovary_nonovary<-data.frame(ovary_achilles_cluster24mean-noovary_achilles_cluster24mean)
...
reduced_ovary_achilles<-select(ovary_achilles_cluster24,vector_delta)
reduced_ovary_achilles
reduced_noovary_achilles<-select(noovary_achilles_cluster24,vector_delta)
...
hist(reduced_ovary_achilles$WNK1)
hist(reduced_noovary_achilles$WNK1)

hist(reduced_ovary_achilles$PAX8)
hist(reduced_noovary_achilles$PAX8)

reduced_ovary_achilles_r<-rownames_to_column(reduced_ovary_achilles, var = "DepMap_ID")

reduced_ovary_achilleslong<-pivot_longer(reduced_ovary_achilles_r,cols = 2:21,names_to= "knockout_genes", values_to="gene_proliferation", values_drop_na=TRUE)
reduced_ovary_achilleslong

## merged prism achilles with prism treat

df_achilles_treat<-merge(reduced_ovary_achilleslong,df_ovary_prism_treat_effective_longc2c4, by="DepMap_ID")
df_achilles_treat

reduced_ovary_achilles_BBK<-select(ovary_achilles_cluster24,c(BRCA1,BRCA2,KRAS))
reduced_ovary_achilles_BBK
reduced_noovary_achilles_BBK<-select(noovary_achilles_cluster24,c(BRCA1,BRCA2,KRAS))
...
pvalues_t_test_achilles24<-data.frame(mapply(function(x,y)t.test(x,y)$p.value,ovary_achilles_cluster24,noovary_achilles_cluster24))

pvalues_t_test_achilles24

##creatig a dataframe with p-vaues and genes
pvalues_t_test_achilles24<-dplyr::rename(pvalues_t_test_achilles24,p_value=mapply.function.x..y..t.test.x..y..p.value..ovary_achilles_cluster24..)
...
```{r}
##ranking the genes
dplyr::arrange(pvalues_t_test_achilles24, BH)
...
vector_genes_difference<-as.vector(c("IGF1R","OR2V2","WNK1","CD200R1L","CPT1A","RAB6A"))
...
```{r}
##exatecan-mesylate,romidepsin,carfilzomib
df_ovary_prism_treat_effective_c2c4
df_noovary_prism_treat_effective_c2c4
reduced_ovary_achilles_genes
reduced_noovary_achilles_genes

cor_achilles_treat= data.frame(row.names=rownames(prism))
