...
> 1. How do different drugs influence ovarian cancer cell lines and which are particularly noticeable? How do these drugs affect the other cell lines?

> 2. Is the sensitivity of different drugs on ovarian cancer cell lines connected to specific cancer-related genes or gene knock-outs?
...
text(-10, 175, "10 % quantile", col = "red3", lwd = 0.5, cex = 0.5)
...
According to the data set prism.cl, the outlier cell line belongs to the most prominent lineage subtype: ovarian adenocarcinoma. This subtype makes up 28 of the 30 lineage subtypes in our data with the remaining two being brenner tumor and ovarian carcinoma. This is why we figured that the lineage subtype could not have been the reason for the cell line forming its own cluster.

We performed several PCAs with the other provided data sets (prism.achilles, prism.exp and prism.cnv) and inspected the behavior of the outlier cell line.
...
In Fig. 5, the contribution of specific drug responses to PC1 for ovarian cell line clusters (see Fig. 4.a.) is shown. Drugs with values close to zero show a similar effect on the proliferation among all cell lines - thus, these are generally effective when administered to ovarian cancer cell lines. This is the case for the following drugs (see highlighted red in Fig. 5).
...
After finding the drugs with the most effective response in regards to the ovarian cancer cell lines, we now continued our analysis by testing if the efficiency of these drugs is dependent on the cancer cell line type or not. In order to test this we repeated many of the same steps we completed for the ovarian cell lines but on the non-ovarian cell lines. We extracted the most negative proliferation value for each drug in each non-ovarian cell line, set the threshold to the 10% quantile, and then extracted the most common MOAs.

As can be seen in Fig. 6, all but one of the most frequently occurring MOAs are the same as the most frequently occurring MOAs in ovarian cancer cell lines (see Fig. 2.b.); however, the frequency with which they occur in the other cell lines differs slightly from the frequency with which they occur in the ovarian cell lines. The NFkB pathway inhibitor is the one MOA that appears among the 45 most frequent MOAs in the non-ovarian cell lines three times but not at all in the 45 most frequent MOAs in the ovarian cell lines. Although NFkB pathway inhibitors do occur in the ovarian cell lines twice, this is not often enough to have made the top 45 which explains why they are not shown in Fig 2.b. NFkB transcription factors are often used to regulate inflammation and immune responses, cell growth, apoptosis, and the expression of certain viral genes [@Gilmore2006]. Due to the importance of the NFkB transcription factors in regulating these processes, NFkB pathway inhibitors are often used in treating chronic inflammation and cancer [@Gilmore2006].
...
As a next step we decided to perform a PCA on all cell lines. We clustered by cell lines in order to test if the ovarian cell lines would form their own distinctive cluster regarding the drug response. As can be seen in Fig. 7, this was not the case. The red dots representing the ovarian cancer cell lines are distributed across the entire plot, rather than concentrated into one distinctive cluster. The fact that the ovarian cell lines did not form their own cluster shows that there seems to be no combining trait among them that would trigger a similar drug response.
...
Additionally to the PCA clustering in Fig. 4.a. and the PCA clustering above, we also conducted a PCA using only non-ovarian cell lines (not shown). As a final step to find out if the drug response is related to the cell line type, we decided to perform a Welch's unpaired two sided t-test, which was subjected a Bonferroni correction. In order to do this we reduced the prism data frame further. It now included the necessary information for those cell lines located in clusters two and four from the PCA clusterings for non-ovarian and ovarian cell lines, respectively. We used this data and the above mentioned PCA clustering for the t-test. The resulting p-value was higher than the confidence level of 0.05 which means that the null hypothesis - that there is no association between the mean value of the drug response in ovarian and non-ovarian cell lines - could not be rejected. This shows once again that the drug response seems to be independent of the cell line type in regards to the ovarian or non-ovarian cell lines.
...
We created a boxplot visualizing the distribution of only the ovarian cell proliferation rates in regards to the drug dose (see Fig. 8). We decided to divide the boxplot into 3 stages: low, medium, and high dose. The boxplot shows an inverse correlation between the drug dose and cell proliferation. Consequently, an increased drug dose leads to reduced proliferation rates.

## Pearson Correlation

Furthermore, we created a correlation matrix with the dosages of the different drugs per ovarian cell line and displayed the Pearson correlation in a heatmap. The blue and the purple squares indicate a high and negative correlation, showing that the higher the drug dosage the smaller the cell proliferation rates become (see Fig. 9).
...
This visualization of the Pearson correlation values shows that there seems to be a strong correlation between increasing dosages and decreasing proliferation values.
...
We extracted the ovarian cell lines of the clusters 1 and 3, 1 and 2, as well as 2 and 3 (see Fig. 4.a.) in order to perform multiple t-tests. We did not include cluster 4 since it consisted of only one cell line which would have provided an unrepresentative result. We performed t-tests on these pairs of clusters with the corresponding data extracted from prism, prism.exp, prism.cnv and prism.achilles. We found that there are significant differences regarding the drug response between the clusters 1-3 and 2-3, respectively.
However, we did not find any major differences concerning the knockdown scores and gene expression rates. Thus, these values do not seem to be the reason for the differing drug responses. Table 1 shows a selection of the drugs that we found to cause significantly different responses between the clusters. We found semaxanib [@KAUR2018129], alvocidib [@doi:10.1080/15384101.2016.1138186], and tricholstatin-a [@CUI2019105973] all to be used in oncology.
...
```{r, echo=FALSE, fig.cap = "Pairwise Plots of Chosen Variables", fig.height=3.5, fig.width=6.5}
# plotting pairwise correlation to visualize the variables
pairs(regression_data_no_nas[,-1], pch=20, col="Lightblue3", cex=0.7)
```

Fig. 10 shows the scatter plots of the explanatory variables. Highly correlated variables should be avoided in a regression model. By choosing PCs to build our regression model, it makes sense that they do not correlate. This is important for successfully computing a linear regression.
...
# residuals should not correlate with our explanatory values 
...
In order to make the linearity assumption, the residuals should be normally distributed [@kronthaler2020data]. We checked this with the help of a q-q plot and saw that this was not fully the case as several values lay outside the qq-line (not shown). In addition to that, the residuals show a correlation with our explanatory variables (`r cor(learning_regression$disulfiram, regression_model$residuals)`) which again revokes the linearity assumption (see. Fig. 11.a.). However, the residuals do have a mean value close to zero (`r mean(regression_model$residuals)`).
Finally, we tried to predict the drug efficiency of disulfiram on our remaining group of ovarian cell lines. As you can see in Fig. 11.b., this worked semi-good. Nonetheless, this does correlate with our results of the previous analyses which showed that there is hardly a correlation between the proliferation values of most ovarian cell lines and the other data sets containing gene expression patterns and knockdown scores. Therefore, trying to predict a drug response based on PCs of these data sets is not a sufficient model in this case. We computed the linear regression model prior to the previous analysis steps, which is why we did not know that it might turn out insufficiently. 
